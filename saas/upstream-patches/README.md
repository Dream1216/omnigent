...
| `0006-pi-gateway-model-catalog.patch` | SaaS Platform | `omnigent/harnesses/pi_native/bridge.py`; `omnigent/harnesses/pi_native/credentials.py` | multi-model Platform gateway projection; secret-free Pi config; allowed-catalog picker regression; delegated Runner Workspace callback routing | Generic upstream gateway-catalog and managed callback-routing proposals pending | `9616bf97` | Remove when upstream publishes all configured inline gateway models to Pi and routes native callback headers through the managed Workspace partition |
