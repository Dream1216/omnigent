...
| `0005-external-host-runtime-bridge.patch` | SaaS Runtime | `omnigent/host/connect.py`; `omnigent/host/identity.py`; `omnigent/server/routes/host_tunnel.py`; `omnigent/stores/host_store.py` | owner-only rotatable token-file contract; tenant tunnel-route isolation; revocation heartbeat; exact expiry boundary | Generic external Host credential/file-source proposal pending | `e179bc42` | Remove when upstream supports file-backed external Host credentials, an explicit tenant route, reconnect rotation, and live revocation fencing |
