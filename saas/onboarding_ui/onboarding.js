(() => {
  ...
  async function signup() {
    ...
    form.addEventListener("submit",async e=>{e.preventDefault();const button=form.querySelector("button"),err=form.querySelector("#error");const body={email:form.email.value.trim().toLowerCase(),display_name:app.querySelector("#display").value.trim()||null,tenant_name:tenant.value.trim(),tenant_slug:tenantSlug.value.trim(),default_space_name:space.value.trim(),default_space_slug:spaceSlug.value.trim(),plan_key:new FormData(form).get("plan"),home_region:app.querySelector("#region").value};const next=JSON.stringify(body);if(next!==fingerprint){fingerprint=next;key=uuid("signup")}button.disabled=true;err.innerHTML="";try{const result=await json("/saas/onboarding/registrations",mutation(body,key));sessionStorage.setItem(PENDING,JSON.stringify({registrationId:result.registration_id,email:body.email}));location.assign(`/signup/verify?registration_id=${encodeURIComponent(result.registration_id)}`)}catch(cause){err.innerHTML=`<div class="alert error">${esc(errorText(cause))}</div>`;button.disabled=false}});
  }
  function registrationId(){return new URLSearchParams(location.search).get("registration_id") || pending()?.registrationId || ""}
  function verify() {
    const id=registrationId(), saved=pending(); app.classList.add("compact");
