...
function ComposerStatusLine({ goal }: { goal: Goal | null }) {
  ...
  const showPreview =
    !!conversationId && serverInfo !== "loading" && serverInfo.login_url === "/saas/login";
  if (!showPlanMode && !showGoal && !showPreview) return null;
    ...
    >
      <div className="flex min-w-0 shrink-0 items-center gap-3">
        {showPreview && conversationId && (
          <PreviewControl key={conversationId} sessionId={conversationId} enabled />
        )}
        {showPlanMode && (
          <span
