...
def test_current_schema_head_requires_a_successor_approval_record() -> None:
    require_current_approval_history(_repo())
    baseline = _baseline()

    assert baseline["approval"]["approved_control_plane_schema_revision"] == "p0s000000012"  # type: ignore[index]
    assert baseline["revision_contract"]["control_plane_schema_revision"] == "p0s000000014"  # type: ignore[index]
