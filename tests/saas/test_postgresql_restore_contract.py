...
def test_canonical_control_plane_rls_inventory_has_exactly_one_hundred_nineteen_tables() -> None:
    assert len(CONTROL_PLANE_RLS_TABLES) == 119
    ...
    assert "saas_model_provider_configurations" in CONTROL_PLANE_RLS_TABLES
    assert "saas_model_provider_configuration_receipts" in CONTROL_PLANE_RLS_TABLES
    assert "saas_model_provider_monthly_budgets" in CONTROL_PLANE_RLS_TABLES
    assert "saas_model_provider_tenant_daily_usage" in CONTROL_PLANE_RLS_TABLES
    assert "saas_model_provider_budget_reservations" in CONTROL_PLANE_RLS_TABLES
    ...
    assert "saas_platform_password_credentials" in CONTROL_PLANE_RLS_TABLES
