...
def test_current_adr_gate_waits_for_p0s12_successor_ci_after_bundle_approval() -> None:
    repo = Path(__file__).resolve().parents[2]
    manifest = json.loads(
        (repo / "saas/acceptance/p0-p6-evidence.json").read_text(encoding="utf-8")
    )
    ...
    assert baseline["approval"]["record"] == current_approval_path
    assert prior_approval_path in gate["evidence"]
    assert approval_path in gate["evidence"]
    assert current_approval_path in gate["evidence"]
